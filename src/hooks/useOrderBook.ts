import { buyQuotesAtom, type OrderBookData, QuotesUpdate, sellQuotesAtom } from "@/atoms/orderBook";
...
export const useOrderBook = ({ operation, action, symbol, group }: UseOrderBookParams): UseOrderBookReturn => {
  ...
  const handleOpen = usePersistentCallback(() => {
    if (status === ConnectionStatus.OPEN) {
      sendMessage(JSON.stringify({
        op: operation,
        args: [`${action}:${symbol}${group && typeof group === 'number' ? '_' + group: ''}`],
      }))
    }
  })
  ...
  const processSnapshot = (data: OrderBookData) => {
    setBuyQuotes(data.bids.slice(0, 8).map((q) => ({ price: q[0], size: q[1] })))
    setSellQuotes(data.asks.slice(0, 8).map((q) => ({ price: q[0], size: q[1] })))
  };
  ...
  const processDelta = (data: OrderBookData) => {
    setBuyQuotes(preBuyQuotes => {
      data.bids.forEach((buyUpdate: QuotesUpdate) => {
        ...
        if (index >= 0 && +buyUpdate[1] > 0) {
          preBuyQuotes[index].size = buyUpdate[1];
        } else {
          preBuyQuotes.push({ price: buyUpdate[0], size: buyUpdate[1] });
        }
      });
      return preBuyQuotes.filter(({ size }) => +size !== 0 ).sort((a, b) => Number(b.price) - Number(a.price)).slice(0, 8)
    })
    ...
    setSellQuotes(preSellQuotes => {
      data.asks.forEach((sellUpdate: QuotesUpdate) => {
        ...
        if (index >= 0 && +sellUpdate[1] > 0) {
          preSellQuotes[index].size = sellUpdate[1];
        } else {
          preSellQuotes.push({ price: sellUpdate[0], size: sellUpdate[1] });
        }
      });
      return preSellQuotes.filter(({ size }) => +size !== 0 ).sort((a, b) => Number(b.price) - Number(a.price)).slice(0, 8)
    })
  };
